...
#         x = x.permute(0, 3, 1, 2).to(memory_format=torch.contiguous_format)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            # autoencode
            aeloss, log_dict_ae = self.loss(qloss, x, xrec, optimizer_idx, self.current_epoch,info,
        ...
        if optimizer_idx == 1:
            # discriminator
            disc_img,log_dict_disc = self.loss(qloss, x, xrec, optimizer_idx, self.current_epoch,info,
                                            last_layer=self.get_last_layer(), split="train")
            
#             self.log("train/disc_img", disc_img, prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        aeloss, log_dict_ae = self.loss(qloss, x, xrec, 0, self.current_epoch,info,
                                        last_layer=self.get_last_layer(), split="val")

        discloss, log_dict_disc = self.loss(qloss, x, xrec, 1, self.current_epoch,info,
                                            last_layer=self.get_last_layer(), split="val")
...
#         disc_map = self.loss.discriminator_img(x)
        ...
        maps = ssim(x,xrec,data_range=1.0,use_pad=True,return_full=True)
    ...
    #         dice = dice_coef(seg, mask).item()
            ...
            grid = torch.cat((x,xrec,mask,res_map,seg))
            rec_path = f'rec_imgs/{self.classname}_test'
...
#         plt.close()
        ...
        area_metrics = cal_metrics(labels, max_areas)

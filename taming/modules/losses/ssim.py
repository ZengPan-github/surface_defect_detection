...
class SSIM(nn.Module):
    def __init__(self, win_size=9,k1=0.01, k2=0.03,gaussian_weights=True,use_sample_covariance=False,sigma=1.5):
        ...
        if gaussian_weights:
            ...
            self.register_buffer('w', _2D_window)
        else:
            self.register_buffer('w', torch.ones(1, 1, win_size, win_size).div(win_size**2))
    ...
    def forward(self, X, Y,alpha=0.5,beta=1.0,data_range=2.0,use_pad=False,mode='reflect',return_full=False):
        ...
        ux = F.conv2d(X_, self.w)
        uy = F.conv2d(Y_, self.w)

        uxx = F.conv2d(X_2, self.w)
        uyy = F.conv2d(Y_2, self.w)
        uxy = F.conv2d(XY, self.w)

...
from taming.modules.discriminator.model import NLayerDiscriminator, weights_init, add_sn, PixelDiscriminator
...
class VQLPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, codebook_weight=1.0, pixelloss_weight=1.0,win_size=9, sigma=1.0,alpha=0.5,beta=1.0,
        ...
        self.ssim_loss = SSIM(win_size=win_size,sigma=sigma)
...
#         self.perceputal_loss = SSIM(win_size=win_size,sigma=sigma)
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
                current_epoch,info,last_layer=None, split="train"):

        rec_loss = self.ssim_loss(inputs,reconstructions,alpha=self.alpha,beta=self.beta) 
#         p_loss = self.perceptual_loss(inputs.expand(-1,3,-1,-1).contiguous(), reconstructions.expand(-1,3,-1,-1).contiguous())
        ...
        if optimizer_idx == 0:
            # generator update
            logits_fake_img = self.discriminator_img(reconstructions.contiguous())
#             logits_fake_mix = self.discriminator_img(mixs.contiguous())
            
            if self.disc_loss_type == 'ls':
                g_loss_img = F.mse_loss(logits_fake_img, torch.ones_like(logits_fake_img))
#                 g_loss_mix = F.mse_loss(logits_fake_mix, torch.ones_like(logits_fake_mix))
            else:
                g_loss_img = -torch.mean(logits_fake_img)
#                 g_loss_mix = -torch.mean(logits_fake_mix)
        ...
        if optimizer_idx == 1:
            ...
            logits_real_img = self.discriminator_img(inputs.contiguous().detach())
            logits_fake_img = self.discriminator_img(reconstructions.contiguous().detach())
#             logits_fake_mix = self.discriminator_img(mixs.contiguous().detach())
...
#                    "{}/logits_fake_mix".format(split): logits_fake_mix.detach().mean(),
                   }
            return d_loss_img, log
